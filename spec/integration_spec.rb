...
describe('books resource', {:type => :feature}) do
  it 'should see books when on /patron/:patron_id/books' do
    ...
    new_patron.save()
    visit("/patron/#{new_patron.id}/books")
    expect(page).to have_content('The Sun Also Rises')
  end

  it 'should see a book when on patron/:patron_id/book/:id' do
    new_book = Book.new({title: 'The Sun Also Rises', author_last: 'Hemingway', author_first: 'Ernest', genre: 'fiction', id: nil})
    new_book.save()
    new_patron = Patron.new({patron_last: 'Smith', patron_first: 'John', phone: '987-867-5309', id: nil})
    new_patron.save()
...
describe('checkout resource', {:type => :feature}) do
  it 'create a checkout when checkout button is clicked' do
    new_book = Book.new({title: 'The Sun Also Rises', author_last: 'Hemingway', author_first: 'Ernest', genre: 'fiction', id: nil})
    new_book.save()
    new_patron = Patron.new({patron_last: 'Smith', patron_first: 'John', phone: '987-867-5309', id: nil})
    new_patron.save()
    ...
    expect(page).to have_content("Due by: #{Checkout.all.first().return_due}")
  end
end
